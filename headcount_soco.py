...
def run(file_path):
    ...
    #############    leo archivos dentro del bucket #################3

    ARCHIVO_S3 = file_path  # Ruta del archivo en S3
    #print("archivo s3")
    #print(ARCHIVO_S3)
    ...
    # SITE
    site = st.sidebar.multiselect("SITE", df2["SITE"].unique())
    if not site:
        df3 = df2.copy()
    else:
   ...
   # SECCION O CC
    seccion = st.sidebar.multiselect(
        "SECCION", df3["SECCION"].unique())



  # FILTRO LOS DATOS SEGÚN HAYA SELECCIONADO
    if not emp and not site and not seccion:
        filtered_df = df
    elif not site and not seccion:
        filtered_df = df[df["EMPRESA"].isin(emp)]
    elif not emp and not seccion:
        filtered_df = df[df["SITE"].isin(site)]

    elif site and seccion:
        filtered_df = df3[df["SITE"].isin(site) & df3["SECCION"].isin(seccion)]

    elif emp and seccion:
        filtered_df = df3[df["EMPRESA"].isin(emp) & df3["SECCION"].isin(seccion)]

    elif emp and site:
        filtered_df = df3[df["EMPRESA"].isin(emp) & df3["SITE"].isin(site)]

    elif seccion:
        filtered_df = df3[df3["SECCION"].isin(seccion)]
    else:
        filtered_df = df3[df3["EMPRESA"].isin(emp) & df3["SITE"].isin(
            site) & df3["SECCION"].isin(seccion)]
    ...
    filtered_df['FECHA_INGRESO'] = filtered_df['FECHA_INGRESO'].replace("00/00/0000", pd.NA)
    filtered_df['FECHA_EGRESO'] = filtered_df['FECHA_EGRESO'].replace("00/00/0000", pd.NA)
    filtered_df['FECHA_EGRESO'] = filtered_df['FECHA_EGRESO'].replace(" ", pd.NA)
    filtered_df['FECHA_NACIMIENTO'] = filtered_df['FECHA_NACIMIENTO'].replace("00/00/0000", pd.NA)
...
# Conversión de fechas con manejo de errores
    filtered_df['FECHA_INGRESO'] = pd.to_datetime(filtered_df["FECHA_INGRESO"], format="%d/%m/%Y", errors='coerce')
    filtered_df['FECHA_EGRESO'] = pd.to_datetime(filtered_df["FECHA_EGRESO"], format="%d/%m/%Y", errors='coerce')
    filtered_df['FECHA_NACIMIENTO'] = pd.to_datetime(filtered_df["FECHA_NACIMIENTO"], format="%d/%m/%Y", errors='coerce')
...
# Seleccionar el rango de periodos usando selectbox
    ...
    #Sprint(filtered_df[['FECHA_NACIMIENTO', 'EDAD']])
